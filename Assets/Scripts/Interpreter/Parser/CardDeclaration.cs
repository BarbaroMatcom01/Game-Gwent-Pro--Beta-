   ...
   using System.Linq;
     public partial class Parser
    {
        private Stmt CardDeclaration()
        {
            ...
            Token descriptionToken = null;
            Expr descriptionValue = null;
            ...
            while (!Check(TokenType.Right_Brace) && !IsAtEnd())
            {
                if (Match(TokenType.Identifier))
                {
                    ...
                    else if (Previous().Value == "Faction")
                    {
                        ParseProperty(ref faction, ref factionValue, "Faction");
                    }
                    else if (Previous().Value == "Power")
                    {
                        ParseProperty(ref power, ref powerValue, "Power");
                    }
                    else if (Previous().Value == "Range")
                    {
                        ParseProperty(ref range, ref rangeValue, "Range");
                    }
                    else if (Previous().Value == "Description")
                    {
                        ParseProperty(ref descriptionToken, ref descriptionValue, "Description");
                    }
                    else if (Previous().Value == "OnActivation")
                    {
            ...
            return new CardStmt(type, typeValue, name, nameValue, faction, factionValue, power, powerValue, range, rangeValue, descriptionToken, descriptionValue, onActivationExprs);
        }
