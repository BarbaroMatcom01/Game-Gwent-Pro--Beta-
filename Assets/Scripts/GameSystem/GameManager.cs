...
public class GameManager : MonoBehaviour
{
   ...
   public void RoundEnd()
   {
      ...
      CardManager.Instance.DestroyCards(Board);
   ...
   public void IdCard()
   {
      Dictionary<int, CardData> Ids = new Dictionary<int, CardData>();

      List<CardData> deckPlayerOne = Board.Instance.PlayerOneSide.Deck.DeckCards;
      List<CardData> deckPlayerTwo = Board.Instance.PlayerTwoSide.Deck.DeckCards;

      int id = 1;

      foreach (CardData cardData in deckPlayerOne)
      {
      ...
      foreach (CardData cardData in deckPlayerTwo)
      {
         while (Ids.ContainsKey(id))
         {
            id++;
